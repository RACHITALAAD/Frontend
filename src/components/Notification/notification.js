    ...
    const fetchNotifications = async () => {
      try {
        ...
        const pendingRes = await axios.get(
          `https://connecthivebackend.onrender.com/api/notifications/notifications/${user.id}/pending`,
        );

        // Fetch accepted connection notifications
        const acceptedRes = await axios.get(
          `https://connecthivebackend.onrender.com/api/notifications/notifications/${user.id}/accepted`,
        );

        // Fetch post notifications
        const postRes = await axios.get(
          `https://connecthivebackend.onrender.com/api/notifications/notifications/${user.id}/posts`,
        );
                ...
                >
                  <div className="notif-card">
                    ...
                    {notif.type === "post_update" && (
                      <span>
                        {moment(notif.createdAt).format("MMMM D, YYYY h:mm A")}
                      </span>
                    )}
                  </div>
                </li>
