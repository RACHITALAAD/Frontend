  ...
  const fetchFeedbacks = async () => {
    try {
      const response = await fetch(
        "https://connecthivebackend.onrender.com/api/feedback/feed",
      );
    ...
    <section className="bg-black text-white py-2 px-8">
      <div className="flex flex-col md:flex-row w-full gap-8 p-8 bg-black">
        ...
        <div className="w-full md:w-1/2 p-6 rounded-xl bg-gray-900 shadow-2xl border border-gray-700">
          ...
          <div
            className="overflow-y-auto max-h-[400px] border-t border-gray-700 
    "
          >
            {feedbackList.length === 0 ? (
              ...
              feedbackList.map((feedback) => (
                <div
                  key={feedback.id}
                  className="p-3 border-b border-gray-700 bg-gray-800 rounded-lg shadow-md my-2"
                >
